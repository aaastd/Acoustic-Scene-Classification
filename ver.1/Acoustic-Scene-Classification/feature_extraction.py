...
def to_hdf5(path, meta, mode, channel):
    with h5py.File(path+'%s.hdf5' % (mode)) as f:
        ...
        for i in range(len(meta)-1):
            ...
            # data[i] = librosa.feature.melspectrogram(y, sr=sr, n_fft=1024, n_mels=40, hop_length=320, power=1, fmax=16000).T
            mel = librosa.feature.melspectrogram(y, sr=sr, n_fft=1024, n_mels=40, hop_length=320, power=1, fmax=16000)
            data[i] = np.log10(np.maximum(mel, 0.000001)).T


# save_path = 'E:/ETRI/DCASE2018/'
save_path = 'F:/DCASE2018/original/'
...
for fold_num in range(1, 5):
    ...
    meta = read_meta('D:/DCASE 2018 Dataset/Development dataset/DCASE2018-task5-dev/evaluation_setup/'
                     'fold%d_evaluate.txt' % fold_num)
    for channel in range(1, 5):
        to_hdf5(save_path, meta, 'fold%d_evaluate' % fold_num, channel)
